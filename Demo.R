...
item_dim = rep(1:Args[['L']], each=Args[['P']]/Args[['L']]) # items' corresponding dimension
item_seq = c(1, Args[['P']]/Args[['L']], Args[['P']]/Args[['L']]+1, Args[['P']]) # the starting and ending item positions for each dimension, ordered sequentially by dimension
# e.g., Specifically: 
...
constants = list(
  ...
  L=Args[['L']],
  M=rep(0,2*L), 
  M_xi=rep(0,V), Sigma_xi=diag(1,V), 
  J_idx=J_idx, 
  item_dim=item_dim, item_seq=item_seq,
  zero_L=rep(0,Args[['L']]), zero_KP=matrix(0,Args[['K']],Args[['P']])
) 
...
for(i in 1:Args[['nchain']]){
  ...
  for(l in 1:Args[['L']]){
    for(v in 1:Args[['V']]){
      tmp_xi[,v,l] = rnorm(N, 0, 1) %>% scale() 
    }
  }
  inits[[i]] =
    list(
      theta = cbind(scale(apply(y[,,1:(Args[['P']]/Args[['L']])],1,sum, na.rm=T)),scale(apply(y[,,1:(Args[['P']]/Args[['L']])],2,sum, na.rm=T)),
                    scale(apply(y[,,(Args[['P']]/Args[['L']]+1):Args[['P']]],1,sum, na.rm=T)),scale(apply(y[,,(Args[['P']]/Args[['L']]+1):Args[['P']]],2,sum, na.rm=T))),
      ...
      log_lambda = matrix(c(-5, .5), 2, Args[['P']]),
      omega = rep(.9, Args[['P']]),
      delta = rep(1, Args[['P']])
    )
}
...
if(calc_dist){
  est_Dist = Est[grep('dist', n_var_est)] %>% array(c(Args[['N']], Args[['N']], Args[['L']]))
}
...
Rep_Omega = post_samples[,grep('omega', n_var)] %>% as.matrix() %>% array(dim=c(nrow(post_samples), Args[['P']]))
...
for(i in 1:Args[['N']]){
  # preference
  FIT_s = Fit_s(y[i,-i,], est_Theta[i,1:Args[['L']]], est_Theta[-i,(Args[['L']]+1):(2*Args[['L']])], est_Beta, est_Tau, est_Lambda, est_Omega, est_Xi[i,,], est_Xi[-i,,])
  FIT_r = Fit_r(y[-i,i,], est_Theta[-i,1:Args[['L']]], est_Theta[i,(Args[['L']]+1):(2*Args[['L']])], est_Beta, est_Tau, est_Lambda, est_Omega, est_Xi[-i,,], est_Xi[i,,])
  ...
  for(reps in 1:nrow(post_samples)){
    ...
    y_rep_s = data_gen_fit(Rep_Theta[reps,i,1:Args[['L']]], Rep_Theta[reps,-i,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,], Rep_Tau[reps,,], Rep_Lambda[reps,,], Rep_Omega[reps,], Rep_Xi[reps,i,,], Rep_Xi[reps,-i,,], 's')[1,,]
    FIT_rep_s = Fit_s(y_rep_s, Rep_Theta[reps,i,1:Args[['L']]], Rep_Theta[reps,-i,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,], Rep_Tau[reps,,], Rep_Lambda[reps,,], Rep_Omega[reps,], Rep_Xi[reps,i,,], Rep_Xi[reps,-i,,], )
    ...
    y_rep_r = data_gen_fit(Rep_Theta[reps,-i,1:Args[['L']]], Rep_Theta[reps,i,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,], Rep_Tau[reps,,], Rep_Lambda[reps,,], Rep_Omega[reps,], Rep_Xi[reps,-i,,], Rep_Xi[reps,i,,], 'r')[,1,]
    FIT_rep_r = Fit_r(y_rep_r,Rep_Theta[reps,-i,1:Args[['L']]], Rep_Theta[reps,i,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,], Rep_Tau[reps,,], Rep_Lambda[reps,,], Rep_Omega[reps,],Rep_Xi[reps,-i,,], Rep_Xi[reps,i,,])
...
for(p in 1:Args[['P']]){
  # preference
  FIT_NET = Fit_net(y[,,p],  Est_Theta[,1:Args[['L']]], Est_Theta[,(Args[['L']]+1):(2*Args[['L']])], Est_Beta[p], Est_Tau[,item_dim[p]], Est_Lambda[,p], Est_Omega[p], Est_Xi)
  
  tmp_Net_p = 0
  FIT_rep_Net_all = vector('numeric', nrow(post_samples))
  for(reps in 1:nrow(post_samples)){
    
    y_rep_Net = data_gen_fit_p(Rep_Theta[reps,,1:Args[['L']]], Rep_Theta[reps,,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,p], Rep_Tau[reps,,item_dim[p]], Rep_Lambda[reps,,p], Rep_Omega[reps,p], Rep_Xi[reps,,,])
    FIT_rep_Net = Fit_net(y_rep_Net, Rep_Theta[reps,,1:Args[['L']]], Rep_Theta[reps,,(Args[['L']]+1):(2*Args[['L']])], Rep_Beta[reps,p], Rep_Tau[reps,,item_dim[p]], Rep_Lambda[reps,,p], Rep_Omega[reps,p], Rep_Xi[reps,,,])
